...
class NetworkManager {
    ...
    func fetchBeachData(beachNum: String, completion: @escaping (Result<WaveHeight, Error>) -> Void ) {
        ...
        let baseURL = "https://apis.data.go.kr/1360000/BeachInfoservice/getWhBuoyBeach?\(APIKey.serviceKey)&dataType=JSON&beach_num\(beachNum)&searchTime=\(currentTime)"
       ...
       // ]
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                ...
                if let item = apiResponse.response.body.item.first {
